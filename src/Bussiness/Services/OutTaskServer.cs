    ...
    public class OutTaskServer : Contracts.IOutTaskContract
    {
            ...
            get
            {
                return OutTasks
                    ...
                    .LeftJoin(WareHouseContract.WareHouses,
                        (outentity, dictionary, warehouse) => outentity.WareHouseCode == warehouse.Code)
                    .InnerJoin(WareHouseContract.Containers, (outentity, dictionary, warehouse, containers) => outentity.ContainerCode == containers.Code)
                    .InnerJoin(EquipmentTypeContract.EquipmentType, (outentity, dictionary, warehouse, containers, equipmentType) => containers.EquipmentCode == equipmentType.Code)
                    .Select((outentity, dictionary, warehouse, containers, equipmentType) => new Dtos.OutTaskDto()
                    {
                        Id = outentity.Id,
                        Code = outentity.Code,
                        ...
                        ContainerCode = outentity.ContainerCode,
        ...
        public IQuery<OutTaskMaterialDto> OutTaskMaterialDtos => OutTaskMaterials
                  ...
                  .Select((outTaskMaterial, material, inTasks, location, supply) => new Dtos.OutTaskMaterialDto
                  {
                      ...
                      CRRCID = outTaskMaterial.CRRCID,
                      WORKSTATIONID = outTaskMaterial.WORKSTATIONID,
                      MERGEID = outTaskMaterial.CRRCID,
                      WAREHBINID = outTaskMaterial.WORKSTATIONID,
                  });
        ...
        public IQuery<OutTaskMaterialLabelDto> OutTaskMaterialLabelDtos => OutTaskMaterialLabels
            ...
            .Select((outMaterialLabel, material, location, tray, supplier) => new OutTaskMaterialLabelDto
            {
                ...
                BoxUrl = location.BoxUrl,
                MaterialUrl=material.PictureUrl,
                CRRCID = outMaterialLabel.CRRCID,
                WORKSTATIONID = outMaterialLabel.WORKSTATIONID,
                MERGEID = outMaterialLabel.CRRCID,
                WAREHBINID = outMaterialLabel.WORKSTATIONID,
            });
        ...
        public DataResult CreateOutTaskEntity(Out entity)
        {
            try
            {
                ...
                // 获取出库仓库
                var outEntity = OutContract.Outs.FirstOrDefault(a => a.Code == entity.Code);

                // 获取出库物料明细
                var outMaterailList = OutContract.OutMaterials.Where(a => a.OutCode == outEntity.Code).ToList();

                // 出库任务物料明细表
                var outTaskMaterialList = new List<OutTaskMaterial>();

                foreach (var item in outMaterailList)
                {
                    // 该物料下发数量
                    decimal sendQuantity = 0;

                    // 获取物料实体
                    var materialEntity = MaterialContract.MaterialDtos.FirstOrDefault(a => a.Code == item.MaterialCode);

                    var query = StockRepository.Query()
                        .Where(a => a.MaterialCode == item.MaterialCode &&
                    ...
                    if (AvailableStock.Sum(a => a.Quantity - a.LockedQuantity) < item.Quantity)
                    {
                        return DataProcess.Failure("物料" + item.MaterialCode + "库存不足");
                    }

                    // 本次待出库的数量
                    decimal needQuantity = item.Quantity;
                    ...
                    {
                        // 如果分配数量还大于出库单行项目数量
                        item.SendInQuantity = item.Quantity;
                        sendQuantity = sendQuantity - item.Quantity;
                    }
                    else
                    {
                        // 如果分配数量还已不足出单行项目数量
                        item.SendInQuantity = sendQuantity;
                        sendQuantity = 0;
                    }
                ...
                if (labelList.Count > 0)
                {
                    ...
                    //var outEntityes = OutContract.OutDtos.FirstOrDefault(a => a.Code == entity.Code);
                    ...
                    foreach (var item in groupList)
                    {
                        ...
                        //var materialEntity = MaterialContract.MaterialDtos.FirstOrDefault(a => a.Code == item.Key.MaterialCode);

                        // 创建一个新的出库任务（OutTask）
                        var outTask = new OutTask()
                        {
                            Status = (int)OutTaskStatusCaption.WaitingForPicking,
                            WareHouseCode = item.Key.WareHouseCode,
                            ContainerCode = item.Key.ContainerCode,
                            IsDeleted=false,
                            OutCode = entity.Code,
                            OutDict = outEntity.OutDict,
                        ...
                        // 创建对应该物料的出库任务明细（OutTaskMaterial）
                        var outMaterialGroup = item.Select(a => new OutTaskMaterial()
                        {
                            OutTaskCode = outTask.Code,
                            Status = (int)OutTaskStatusCaption.WaitingForPicking,
                            ...
                            SuggestLocation = a.LocationCode,
                            SuggestContainerCode = item.Key.ContainerCode,
                            SuggestTrayId = a.TrayId,
                        ...
                        // 插入出库任务明细（OutTaskMaterial）和出库任务标签（OutTaskMaterialLabel）

                        foreach(var outTaskMaterial in outMaterialGroup)
                        {
                            if (!OutTaskMaterialRepository.Insert(outTaskMaterial))
                            {
                                return DataProcess.Failure(string.Format("出库库任务明细{0}新增失败", entity.Code));
                            }
                        }

                        //var outMaterialLabelGrop = item.Select(a => new OutTaskMaterialLabel()
                        //{
                        //    TaskCode = outTask.Code,
                        //    BatchCode = a.BatchCode,
                        //    BillCode = entity.BillCode,
                        //    IsDeleted = false,
                        ...
                        //    OutMaterialId = 0,
                        //    Quantity = a.Quantity,
                        //    Status = (int)Bussiness.Enums.OutStatusCaption.SendedPickOrder,
                        //    WareHouseCode = a.WareHouseCode,
                        //    AreaCode = a.AreaCode,
                        //    XLight = a.XLight,
                        //    YLight = a.YLight,
                        ...
                        //foreach(var outMaterialLabel in outMaterialLabelGrop)
                        //{
                        //    if (!OutTaskMaterialLabelRepository.Insert(outMaterialLabel))
                        //    {
                        //        return DataProcess.Failure(string.Format("出库任务标签{0}新增失败", entity.Code));
                        //    }
                        //}
        ...
        public DataResult RemoveOutTask(int id)
        {
            ...
            // 获取入库单
            var outEntity = OutContract.OutRepository.GetEntity(a => a.Code == entity.OutCode);
        ...
        public DataResult ConfirmHandPicked(OutTaskMaterialDto entityDto)
        {
            ...
            OutTaskMaterialDto outEntityes = OutTaskMaterialDtos.FirstOrDefault(a => a.OutCode == entity.OutCode);
            ...
            try
            {
                ...
                {
                    MaterialId = outEntityes.WAREHBINID,
                    WorkStationId = outEntityes.WORKSTATIONID,
                    WarehBinId = outEntityes.CRRCID,
                ...
                foreach (JObject item in jsonArray)
                {
                    ...
                    {
                        jsonObject[property.Key] = property.Value.ToString();
                    }
                }
                ...
                byte[] btBodys = Encoding.UTF8.GetBytes(updatedJson);
