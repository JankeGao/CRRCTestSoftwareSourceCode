    ...
    public class OutServer : Contracts.IOutContract
    {
      ...
      //  public IRepository<OutMaterialLabel, int> OutMaterialLabelRepository { get; set; }
            ...
            .Select((outMaterial, material, outs) => new Dtos.OutMaterialDto
        {
            ...
            CRRCID = outMaterial.CRRCID,
            WORKSTATIONID  = outMaterial.WORKSTATIONID,
            MERGEID = outMaterial.MERGEID,
            WAREHBINID = outMaterial.WAREHBINID,
            });
            ...
            .Select((outentity, dictionary, warehouse) => new Dtos.OutDto()
        {
            ...
            WORKSTATIONID = outentity.WORKSTATIONID,
            MERGEID = outentity.CRRCID,
            WAREHBINID = outentity.WORKSTATIONID,
            });
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                ...
                foreach (var dict in resultList)
                {
                    ...
                    inMaterialObj = new OutMaterial
                    {
                        ...
                        CRRCID = dict["WAREH_BIN_ID"].ToString(),
                        WORKSTATIONID = dict["WORK_STATION_ID"].ToString(),
                        MERGEID = dict["MERGE_ID"].ToString(),
                        WAREHBINID = dict["MATERIAL_ID"].ToString(),
                        Status = 0,
                        BatchCode = "",

                    };

                    if (!OutMaterials.Any(a => a.CRRCID == inMaterialObj.CRRCID))
                    {

                        if (currentBillCode != null && dict["WAREH_BIN_ID"].ToString() != currentBillCode && dict["BILL_CODE"].ToString() != currentCode)
                        {
                            continue;
                        }
                        else
                        {
                            currentBillCode = dict["WAREH_BIN_ID"].ToString();
                            currentCode = dict["BILL_CODE"].ToString();
                ...
                OutRepository.UnitOfWork.TransactionEnabled = true;
                {
                    ...
                    if (entity.AddMaterial != null && entity.AddMaterial.Count() > 0)
                    {
                        ...
                        foreach (OutMaterial item in entity.AddMaterial)
                        {
                            ...
                            //item.CRRCID = inMaterialObj.CRRCID;
                            //item.WORKSTATIONID = inMaterialObj.WORKSTATIONID;
                            //item.MERGEID = inMaterialObj.MERGEID;
                            //item.WAREHBINID = inMaterialObj.WAREHBINID;
                ...
                try
                {
                    ...
                    OutMaterialDto outMaterial1 = OutContract.OutMaterialDtos.FirstOrDefault(a => a.OutCode == entity.Code);
                        ...
                        foreach (Stock stock in AvailableStock)
                        {
                            ...
                            if (aviQuantiy >= needQuantity)
                            {
                                OutTaskMaterialLabel label = new OutTaskMaterialLabel
                                {
                                    ...
                                    CRRCID = outMaterial1.CRRCID,
                                    WORKSTATIONID = outMaterial1.WORKSTATIONID,
                                    MERGEID = outMaterial1.MERGEID,
                                    WAREHBINID = outMaterial1.WAREHBINID,
                                };
                                labelList.Add(label);
                    ...
                    if (labelList.Count > 0)
                    {
                        ...
                        foreach (var item in groupList)
                        {
                            ...
                            { a.WareHouseCode, a.TrayId, a.ContainerCode, a.LocationCode, a.MaterialCode, a.BatchCode, a.CRRCID, a.WORKSTATIONID,a.MERGEID,a.WAREHBINID }).ToList();
                            ...
                            foreach (var OutMaterial in materialList)
                            {
                                ...
                                var OutMaterialEntity = new OutTaskMaterial()
                                {
                                    ...
                                    CRRCID = OutMaterial.Key.CRRCID,
                                    WORKSTATIONID = OutMaterial.Key.WORKSTATIONID,
                                    MERGEID = OutMaterial.Key.MERGEID,
                                    WAREHBINID = OutMaterial.Key.WAREHBINID
                                };
